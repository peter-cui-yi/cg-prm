...
To test whether counterfactual supervision teaches genuine grounding rather than synthetic artifact detection, we build the empirical study around two complementary settings: **CLEVR**, which provides exact symbolic verification under low annotation noise, and **DocVQA**, which provides a realistic document-grounding setting with OCR-backed evidence. The evaluation includes a human-written grounding challenge set, a cross-corruptor split, free-form robustness tests, and fixed-budget reranking analysis. Rather than presenting CG-PRM as a new agent or an outcome-optimization method, we position it as a method for improving the supervision signal available to multimodal verifiers. This draft lays out the method, problem formulation, and evaluation protocol that will support the final paper once experiments are completed.
...
To answer this question cleanly, we use **CLEVR** as a low-noise causal anchor and **DocVQA** as a realistic visual-text grounding benchmark. We evaluate on synthetic corruptions, an independent cross-corruptor split, a human-written grounding challenge set, and free-form traces with deterministic segmentation rules. We treat fixed-budget reranking as a stress test of verifier utility rather than as the main claim of the paper.
...
For `CLEVR`, the canonical reasoning pattern is:

`identify relevant objects -> state relation/count/attribute -> derive answer`
...
For `CLEVR`, verification is based on symbolic scene metadata.
...
In `DocVQA`, categories (1) and (3) are the most natural. In `CLEVR`, categories (2) and (3) dominate, while (1) appears as incorrect attribute binding over correctly identified objects.
...
We use two primary benchmarks.

**CLEVR** serves as the low-noise causal anchor because step correctness can be checked exactly through symbolic scene structure.  
...
`ChartQA` is intentionally left outside the core paper story and can be added later as supplementary transfer evidence if the main results stabilize early.
...
The gold evaluation artifact contains `100` matched trace pairs per benchmark. Each pair consists of:
...
### Table 1. Main verifier comparison on primary benchmarks

| Method | CLEVR step AUROC | CLEVR human pair acc | DocVQA step AUROC | DocVQA human pair acc | Notes |
|---|---:|---:|---:|---:|---|
...
### Table 2. Cross-corruptor and free-form robustness

| Method | Cross-corruptor | Canonical | Lightly structured | Natural free-form |
|---|---:|---:|---:|---:|
| Clean-only visual PRM | TODO | TODO | TODO | TODO |
| CG-PRM | TODO | TODO | TODO | TODO |
...
Another limitation is dependence on verifiable subsets. For `DocVQA`, only a subset of examples admit reliable OCR-backed verification. This is a necessary compromise for initial scientific identification, but it raises external-validity questions that the paper must report explicitly through coverage and subset-bias analysis.
