...
# Kill any stuck torch processes and free port 29500
echo "Cleaning up any stuck processes..."
pkill -f torchrun 2>/dev/null || true
sleep 2

# Activate nips27 environment for training
echo "Activating nips27 environment..."
if [ -f ~/anaconda3/bin/activate ]; then
    source ~/anaconda3/bin/activate nips27
else
    echo "Warning: Could not activate nips27 environment"
fi
...
echo "Start: $(date)"
CUDA_VISIBLE_DEVICES=$GPU_ID torchrun --nproc_per_node=1 --master_port=29501 scripts/training/train_lora.py \
...
echo "Start: $(date)"
CUDA_VISIBLE_DEVICES=$GPU_ID torchrun --nproc_per_node=1 --master_port=29502 scripts/training/train_lora.py \
